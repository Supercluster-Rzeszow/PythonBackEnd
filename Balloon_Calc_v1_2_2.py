...
def MainLoop():
    ...
    for i in range(1,len(TimeVector)):
        ...
        if flag == 1:
            v[i] = parachute_fall_rate_0 / (math.sqrt(rho[i]/rho[0]))
            calka_h         = calka_h + (v[i] + v[i-1]) * 0.5 * SimBase
            h[i]            = calka_h
        ...
        if h[i] <= initial_altitude:
            landing_time = i * SimBase
            print(h[i])
            break


    output_data = pd.DataFrame(np.transpose(np.array([TimeVector, h, v, a, T, p, rho, g, F_G, F_B, F_D, V_b, A_b, r, latitude, longitude])),
                               columns=['Time [s]', 'Height [m]', 'Vertical speed [m/s]',
                                        ...
                                        'Balloon volume [m^3]', 'Balloon cross-section area [m^2]', 'Balloon radius [m]',
                                        'Latitude [deg]', 'Longitude [deg]'])

    return output_data, pop_time, pop_height, landing_time
#-----------------------------------------------
...
output_data, pop_time, pop_height, landing_time = MainLoop()
print(pop_height)
print(landing_time)
#print(output_data["Height [m]"])


plt.plot(output_data["Time [s]"]/60, output_data["Height [m]"])
#plt.plot(output_data["Time [s]"]/60, output_data["Balloon radius [m]"])
