...
def SpeedOfSound(Th):
    ...
    return a_h
#-----------------------------------------------
def MainLoop(wind_altitude_array, wind_direction_array, wind_speed_array, C_d, V_b):
    height  =   Height()
    GM      =   3.986004e+14
    ER      =   6.3781e+06
    SimBase =   0.1
    ...
    array1 = np.transpose(np.array([TimeVector, h, v, a, T, p, rho,
                                    g, F_G, F_B, F_D, V_b, A_b, r,
                                    latitude, longitude, gravity_noise, buoyancy_noise, drag_noise, north_velocity, east_velocity]))
    array1 = array1[0:i]
    array2 = array1[::200][:]
    ...
    output_data = pd.DataFrame(array2,columns=['Time [s]', 'Height [m]', 'Vertical speed [m/s]',
                                               ...
                                               'Balloon volume [m^3]', 'Balloon cross-section area [m^2]',
                                               'Baloon radius [m]', 'Latitude [deg]', 'Longitude [deg]',
                                               'Gravity Measured [N]', 'Buoyancy Measured [N]', 'Drag Measured [N]'
                                               'North speed [m/s]', 'East speed [m/s]'])
...
def run(C_d, V_b, BaloonMass, PayloadMass, r_pop, initial_altitude, initial_latitude, initial_longitude, parachute_fall_rate_0):
    ...
    output_data, pop_time, pop_height, north_velocity, east_velocity = MainLoop(wind_altitude_array,
