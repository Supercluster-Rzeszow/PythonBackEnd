...
def MainLoop():
    ...
    for i in range(1,len(TimeVector)):
        ...
        if flag == 1:
            parachute_fall_rate[i] = parachute_fall_rate_0 / (math.sqrt(rho[i]/rho[0]))
            v[i] = parachute_fall_rate[i]
            calka_h         = calka_h + ((-1) * v[i] + (-1) * v[i-1]) * 0.5 * SimBase
            h[i]            = calka_h
    ...
    output_data = pd.DataFrame(np.transpose(np.array([TimeVector, h, v, a, T, p, rho, g, F_G, F_B, F_D, V_b, A_b, r, latitude, longitude, landing_time])),
                               columns=['Time [s]', 'Height [m]', 'Vertical speed [m/s]',
                                        ...
                                        'Latitude [deg]', 'Longitude [deg]', 'Landing Time [s]'])

    return output_data, pop_time, pop_height, landing_time, north_velocity, east_velocity
#-----------------------------------------------
...
output_data, pop_time, pop_height, landing_time, north_velocity, east_velocity = MainLoop()
print(pop_height)
print(landing_time)
